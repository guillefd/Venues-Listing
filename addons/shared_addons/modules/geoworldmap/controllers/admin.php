...
class Admin extends Admin_Controller {
	...
	public function __construct()
	{
		...
		if ($countries = $this->geo_countries_m->get_all())
		{
			...
			{
				$_countries[$country->CountryId] = $country->Country;
			}
		}
		...
		if ($timezones = $this->geo_countries_m->timezones_get_all())
		{
			...
			{
				$_timezones[$tz->timeZoneId] = $tz->timeZoneId.' '.$tz->GMT_offset;
			}
		}

		//Carga lista de select boxes
		$this->template
			 ->set('countries',$_countries)
			 ->set('timezones',$_timezones);       
	}	      
	...
	public function index()
	{
        ...
        if($this->input->post('f_region')){ 
            if ($regions = $this->geo_regions_m->get_many_by($this->input->post('f_region')))
            {
                ...
                {
                        $_regions[$region->RegionID] = $region->Region;
                }
            }
        }else
            {
		...
		$this->input->is_ajax_request() ? $this->template->build('admin/tables/cities') : $this->template->build('admin/index');
	}
	...
	public function ajax_filter()
	{
        ...
        //query with limits
        $results = $this->geo_cities_m->limit($pagination['limit'], $pagination['offset'])->search('results',$post_data);
