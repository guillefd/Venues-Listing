...
<?php else: ?>
	<h4><?php echo sprintf(lang('features:edit_title'), $feature->name); ?></h4>
<?php endif; ?>
</section>
				...
				<fieldset>	
					<ul>
						...
						<li class="even">
							...
							<div class="input"><?php echo form_input('short_name', $feature->short_name, ' class="small" ') ?></div>
						</li>   
			            <li class="even">
							<label for="cat_product_id"><?php echo lang('features:cat_product'); ?> <span>*</span></label>
							<div class="input">
			                    <?php echo form_dropdown('cat_product_id',array(''=>'') + $dd->cat_products_multiarray, set_value('cat_product_id',$feature->cat_product_id),' data-placeholder="'.lang('features:select_cat').'"id="cat_product_id"') ?>
			                </div>				
						</li>                         
						<li class="even">
							<label for="cat_feature_id"><?php echo lang('features:cat_feature'); ?> <span>*</span></label>
							<div class="input">
				                <?php echo form_dropdown('cat_feature_id',array(''=>'') + $dd->cat_features_array, set_value('cat_feature_id',$feature->cat_feature_id),'data-placeholder="'.lang('features:select_cat').'" id="cat_feature_id"') ?>
			                    [ <?php echo anchor('admin/products/features/cat_feature_form', lang('features:add_cat'), 'target="_blank" id="btn_new_cat_feature" '); ?> ]                                   
				            </div>                                                         
						</li>                    				           
						<li class="even">
							<label for="usageunit_id"><?php echo lang('features:usageunit_value'); ?> <span>*</span></label>
							<div class="input">
			                    <?php echo form_dropdown('usageunit_id',array(''=>'') + $dd->usageunit_array,set_value('usageunit_id',$feature->usageunit_id),'data-placeholder="'.lang('features:select_usageunit').'" id="usageunit_id"') ?><br>                                                   
