...
class Admin extends Admin_Controller
{
    ...
    */
    ...
    {
	    //week array
		$_week_array =$this->config->item('dd_week_days'); 
	    //hour array
		$_hour_array =gen_lista_horas(); 
	    //condicion iva array
	    $_cond_iva_array = $this->config->item('dd_cond_iva');
	    //account_type array
	    $_account_type_array = $this->config->item('dd_account_type');
	    //acount payment mode
	    $_forma_de_pago_array = $this->config->item('dd_payment_mode');  
	    //	
	    $this->template
	    	->set('week_array', $_week_array)
	    	->set('hour_array', $_hour_array)
	    	->set('cond_iva_array',$_cond_iva_array)
	    	->set('account_type_array', $_account_type_array)
	    	->set('forma_de_pago_array', $_forma_de_pago_array);          
    }
	...
	public function index()
	{
		...
		$this->input->is_ajax_request() ? $this->template->build('admin/partials/accounts') : $this->template->build('admin/index');                
	}        
	...
	public function create()
	{
		...
		{                      
            $account->$field['field'] = set_value($field['field']);                                
		}		
	...
	public function edit($id = 0)
	{
        ...
        {    
            //consulta SQL
            $_account = $this->accounts_m->get($id); 
            if($_account == FALSE)
            {
                $this->session->set_flashdata('error', lang('accounts:error_id_empty'));
                redirect('admin/accounts');
        ...
        $_city = $this->geoworldmap->getCityByID($_account->CityID);
        $_account->City = $_city->City;            
