...
class Admin_contacts extends Admin_Controller
{
	...
	public function index()
	{
		...
		$this->input->is_ajax_request() ? $this->template->build('admin/contacts/partials/contacts') : $this->template->build('admin/contacts/index');           
        }        
	...
	public function create()
	{
		...
		{                      
            $contact->$field['field'] = set_value($field['field']);                                
		}
	...
	public function edit($id = 0)
	{
            ...
            {    
                //consulta SQL
                $_contact = $this->contacts_m->get($id); 
                if($_contact == FALSE)
                {
                    $this->session->set_flashdata('error', lang('accounts:error_id_empty'));
                    redirect('admin/accounts/contacts');
            ...
            $_contact = $this->_convertIDtoText($_contact);
            //Use Geoworldmap library 
            $city = $this->geoworldmap->getCityByID($_contact->CityID);
            $_City = $city->City;
